...
def safe_proc_read(pid):
    ...
    try:
        if not proc_path.resolve(strict=True).is_relative_to("/proc/"):
            return None
...
def ensure_venv_blue_green():
    ...
    subprocess.run(["apt-get", "update"], env=env, stdout=None, stderr=None)
    ...
    if install_proc.returncode != 0:
        clog.warning("OS package dependency alignment variance detected.")
    ...
    for pkg in base_packages:
        ...
        subprocess.run([python_exe, "-m", "pip", "install", "--no-cache-dir", pkg], check=True)

    clog.info("Synchronizing tensor operations backend...")
    try:
        subprocess.run([python_exe, "-m", "pip", "install", "--no-cache-dir",
                        "torch", "torchvision", "--index-url", "https://download.pytorch.org/whl/cpu"], check=True)
        subprocess.run([python_exe, "-m", "pip", "install", "--no-cache-dir", "tntorch"], check=True)
        ...
        for path in ["/etc/machine-id", "/sys/class/dmi/id/product_uuid", "/proc/sys/kernel/random/boot_id"]:
            try:
                with open(path, "r") as f:
                    hardware_signatures.append(f.read().strip())
            except Exception:
                pass
        signature_payload = "".join(hardware_signatures).encode('utf-8')
        if not signature_payload:
            signature_payload = socket.gethostname().encode('utf-8')
...
def find_free_port(start_port=9100, max_attempts=100):
    """Find an available port starting from start_port, return port number."""
...
def kill_zombies():
    """
    Safely terminate orphaned instances of the kinematics core, excluding
    the current process, its parent (sudo/shell), and any process in the
    same process group to avoid self‑destruction.
    """
    try:
...
# ── Permissions Verification and Repair ──────────────────────────────
def verify_and_fix_permissions():
    """Check and repair ownership/perms of critical files and directories."""
    critical_paths = [
        (ZARQA_HOME, 0o755, 'zarqa-math', 'zarqa-math'),
        (os.path.dirname(KEY_FILE), 0o750, 'root', 'zarqa-math'),
        (KEY_FILE, 0o640, 'root', 'zarqa-math'),
        (CONFIG_PATH, 0o640, 'root', 'zarqa-math'),
        (STATE_DIR, 0o755, 'zarqa-math', 'zarqa-math'),
    ]
    try:
        import grp
    except ImportError:
    ...
    for path, mode, owner, group in critical_paths:
        if not os.path.exists(path):
            if path == CONFIG_PATH:
                continue  # will be created later
        ...
        try:
            ...
            if grp:
                ...
                if stat_info.st_uid != target_uid or stat_info.st_gid != target_gid:
                    ...
                    clog.info(f"Fixed ownership of {path} to {owner}:{group}")
                if (stat_info.st_mode & 0o777) != mode:
                    os.chmod(path, mode)
                    clog.info(f"Fixed permissions of {path} to {mode:o}")
        except Exception as e:
            clog.warning(f"Could not verify permissions for {path}: {e}")
...
def deploy(script_path):
    ...
    # Dynamically find a free port for metrics if 9100 is occupied
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.bind(('127.0.0.1', METRICS_PORT))
            # Port is free, keep it
            clog.info(f"Metrics port {METRICS_PORT} is available.")
    except OSError:
        clog.warning(f"Metrics port {METRICS_PORT} is in use. Scanning for free port...")
        METRICS_PORT = find_free_port(METRICS_PORT + 1)
        clog.info(f"Will use metrics port {METRICS_PORT} instead.")
    ...
    clog.info("Verifying and repairing system permissions...")
...
class UnderactuatedController:
    ...
    def _compute_controller(self):
        ...
        def check_gamma(g):
            ...
            if not np.all(np.real(eigvals) > 1e-6):
                return False
        ...
        if self.n > len(base_poles):
            base_poles += [-15.0] * (self.n - len(base_poles))
...
def daemon_loop(interval=300):
    global global_reload_flag, current_config, current_arm, current_controller, METRICS_PORT
    # If metrics port is in use, find a free one
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.bind(('127.0.0.1', METRICS_PORT))
    except OSError:
        clog.warning(f"Metrics port {METRICS_PORT} is in use. Scanning for free port...")
        METRICS_PORT = find_free_port(METRICS_PORT + 1)
        clog.info(f"Using metrics port {METRICS_PORT}.")
